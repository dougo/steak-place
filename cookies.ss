;;;
;;; <cookies.ss> ---- HTTP cookies library
;;; Time-stamp: <02/05/07 21:02:33 solsona>
;;;
;;; Copyright (C) 2002 by Francisco Solsona. 
;;;
;;; This file is part of XFC.

;;; XFC is free software; you can redistribute it and/or
;;; modify it under the terms of the GNU Lesser General Public
;;; License as published by the Free Software Foundation; either
;;; version 2.1 of the License, or (at your option) any later version.

;;; XFC is distributed in the hope that it will be useful,
;;; but WITHOUT ANY WARRANTY; without even the implied warranty of
;;; MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
;;; Lesser General Public License for more details.

;;; You should have received a copy of the GNU Lesser General Public
;;; License along with XFC; if not, write to the Free Software
...
(module cookies mzscheme
  ...
  (define set-cookie
    (lambda (name value)
      (unless (and (cookie-string? name #f)
		   (cookie-string? value))
	(raise (make-cookie-error (format "Invalid NAME/VALUE pair: ~a / ~a" name value))))
  ...
  (define print-cookie
   ...
   (define cookie:add-comment
	 ...
	 (raise (make-cookie-error (format "Invalid comment: ~a" comment))))
       (unless (cookie? cookie)
	 (raise (make-cookie-error (format "Cookie expected, received: ~a" cookie))))
   ...
   (define cookie:add-domain
	 ...
	 (raise (make-cookie-error (format "Invalid domain: ~a" domain))))
       (unless (cookie? cookie)
	 (raise (make-cookie-error (format "Cookie expected, received: ~a" cookie))))
   ...
   (define cookie:add-max-age
	 ...
	 (raise (make-cookie-error (format "Invalid Max-Age for cookie: ~a" seconds))))
       (unless (cookie? cookie)
	 (raise (make-cookie-error (format "Cookie expected, received: ~a" cookie))))
   ...
   (define cookie:add-path
     (lambda (cookie path)
       (unless (string? path)
	 (raise (make-cookie-error (format "Invalid path: ~a" path))))
       (unless (cookie? cookie)
	 (raise (make-cookie-error (format "Cookie expected, received: ~a" cookie))))
   ...
   (define cookie:secure
	 ...
	 (raise (make-cookie-error (format "Invalid argument (boolean expected), received: ~a" secure?))))
       (unless (cookie? cookie)
	 (raise (make-cookie-error (format "Cookie expected, received: ~a" cookie))))
   ...
   (define cookie:version
	 ...
	 (raise (make-cookie-error (format "Unsupported version: ~a" version))))
       (unless (cookie? cookie)
	 (raise (make-cookie-error (format "Cookie expected, received: ~a" cookie))))
