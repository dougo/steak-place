;; Implementation of md5 by Jens Axel Søgaard
;; Changes: module packaging; some files have been commented or
;; uncommented to be specific to PLT Scheme.
...
;;  (define (ior b1 b2)
;;    (if (> (+ b1 b2) 0) 1 0))
;;  (map-bitwise ior w1 w2))

;;; word-xor : word word -> word
;;;  bit-wise exclusive or
;;(define (word-xor w1 w2)
;;  (define (xor b1 b2)
;;    (if (= (+ b1 b2) 1) 1 0))
;;  (map-bitwise xor w1 w2))
...
;;  (define (land b1 b2)
;;    (if (= (+ b1 b2) 2) 1 0))
;;  (map-bitwise land w1 w2))

;;; word<<< : word 0..31 -> word
;;;  rotate the 32-bit word s bit positions to the left
;;(define (word<<< n s)
;;  (define (new-index i)
;;    (modulo (- i s) 32))
;;  (let* ((v   (list->vector (word->bits n)))
;;         (ref (lambda (i) (vector-ref v i))))
;;    (bits->integer (map ref (map new-index (iota 0 31))))))
...
(define (round1  a b c d X)
  (md5-round 1  a b c d X))
(define (round2  a b c d X)
  (md5-round 2  a b c d X))
(define (round3  a b c d X)
  (md5-round 3  a b c d X))
(define (round4  a b c d X)
  (md5-round 4  a b c d X))
